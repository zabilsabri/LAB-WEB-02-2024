...
@section('content')
    ...
    <form action="{{ route('categories.update', $category) }}" class="bg-white p-6 rounded-lg shadow-md" method="POST">
        @csrf
        @method('PUT')
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="description" id="desciption" value="{{ $category->description }}" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" />
        </div>
